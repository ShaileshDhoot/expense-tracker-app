function addMonthData(element) {
  ...
  window.addEventListener("DOMContentLoaded",async () => {
    try{
      ...
      const response = await axios.get("http://3.83.64.232/expense/all", { headers: { "Authorization": token } })          
...
async function download(){
  try{
    ...
    //console.log(token);
    const response = await axios.get('http://3.83.64.232/expense/download', { headers: {"Authorization" : token} })        
